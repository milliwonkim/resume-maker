...
import { getResumes, getSections } from '@/lib/notion-db';
import { replaceResumeWithSectionsByTitle } from '@/lib/supabase-db';
...
async function runMigration(databaseIdInput: string | undefined) {
  ...
  if (!databaseId) {
    return Response.json(
      { error: '마이그레이션할 Notion 데이터베이스 ID가 필요합니다.' },
      { status: 400 }
    );
  }

  try {
    const resumes = await getResumes(token, databaseId);
    let sectionCount = 0;

    for (const resume of resumes) {
      const sections = await getSections(token, resume.id);
      sectionCount += sections.length;
      await replaceResumeWithSectionsByTitle(resume, sections);
    }
