...
export function HeaderSection({ content, layout, onChange }: Props) {
  ...
  if (layout === 'layout1') {
    return (
      <div className="text-center py-6 border-b-2 border-gray-800">
        <EditableField
          {...f('name')}
          tag="h1"
          placeholder="이름"
        ...
        <EditableField
          ...
          tag="p"
          placeholder="직함"
          className="text-xl text-blue-600 mt-1 block"
        />
  ...
  if (layout === 'layout2') {
    return (
      <div className="py-6 border-b-2 border-gray-800">
        <EditableField
          {...f('name')}
          tag="h1"
          placeholder="이름"
  ...
  // layout3: two-column
  return (
    <div className="py-6 border-b-2 border-gray-800 flex items-center justify-between gap-8">
      ...
      <div className="text-right text-sm text-gray-600 space-y-1">
        <div><EditableField {...f('email')} placeholder="이메일" /></div>
        <div><EditableField {...f('phone')} placeholder="전화번호" /></div>
        <div><EditableField {...f('location')} placeholder="위치" /></div>
        <div><EditableField {...f('github')} placeholder="GitHub (선택)" /></div>
        <div><EditableField {...f('linkedin')} placeholder="LinkedIn (선택)" /></div>
      </div>
    </div>
  );
}
