...
function SectionSkeleton({ label }: { label: string }) {
  return (
    <div className="no-print relative pt-10 sm:pt-8 rounded-lg animate-pulse" aria-live="polite" aria-label={`${label} 추가 중`}>
      <div className="text-sm font-bold uppercase tracking-widest text-gray-300 mb-3 border-b border-gray-200 pb-1">
        {label} 추가 중...
      </div>
...
export const ResumeEditor = forwardRef<ResumeEditorRef, Props>(function ResumeEditor(
  { resumeId, autoSave, onPendingChange },
  ref
) {
  const { sections, addSection, removeSection, updateSectionLayout, updateSectionContent, moveSectionUp, moveSectionDown, reorderSection, setSections, setIsSaving } =
    useResumeStore();

  const saveTimers = useRef<Map<string, ReturnType<typeof setTimeout>>>(new Map());
  const pendingSaves = useRef<Map<string, { layout?: string; content?: SectionContent }>>(new Map());
  ...
  const [addingSectionType, setAddingSectionType] = useState<SectionType | null>(null);
  const [sectionAddError, setSectionAddError] = useState<string | null>(null);
  const visibleSections = useMemo(() => sections.filter((s) => s.id), [sections]);
  const isAddingSection = addingSectionType !== null;

  const saveOrder = useCallback(async () => {
    const current = useResumeStore.getState().sections;
    const savedSections = current.filter((s) => s.id && !isTemporarySectionId(s.id));
  ...
  const scheduleSave = useCallback(
    (sectionId: string, payload: { layout?: string; content?: SectionContent }) => {
      ...
      const timer = setTimeout(async () => {
        setIsSaving(true);
        try {
          const latestPayload = pendingSaves.current.get(sectionId) ?? payload;
  ...
  const flushPendingSaves = useCallback(async () => {
    ...
    const createdSections = currentSections.filter((section) => createdSectionIds.has(section.id));
    ...
    try {
      for (const sectionId of deletedSectionIds) {
        await fetch(`/api/resumes/${resumeId}/sections/${sectionId}`, { method: 'DELETE' });
      }
      ...
      if (createdSectionsByTempId.size > 0) {
        ...
        setSections(latestSections.map((section) => createdSectionsByTempId.get(section.id) ?? section));
      }

      for (const [sectionId, payload] of entries) {
        if (isTemporarySectionId(sectionId) || deletedSectionIds.includes(sectionId)) continue;
        await fetch(`/api/resumes/${resumeId}/sections/${sectionId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
      ...
      entries.forEach(([sectionId]) => pendingSaves.current.delete(sectionId));
      deletedSectionIds.forEach((sectionId) => pendingDeletedSectionIds.current.delete(sectionId));
      createdSections.forEach((section) => pendingCreatedSectionIds.current.delete(section.id));
  ...
  useImperativeHandle(ref, () => ({
    ...
    markAllDirty: () => {
      ...
      currentSections
        ...
        .forEach((section) => {
          pendingSaves.current.set(section.id, { content: section.content, layout: section.layout });
        });
  ...
  const dispatchSave = useCallback(
    (sectionId: string | undefined, payload: { layout?: string; content?: SectionContent }) => {
      if (!sectionId) return;
      if (autoSave) {
  ...
  const handleMoveUp = useCallback(
    async (sectionId: string) => {
      ...
      if (autoSave) {
        onPendingChange(true);
        setTimeout(() => {
          void saveOrder().finally(() => onPendingChange(hasPendingChanges()));
        }, 0);
      } else {
        hasPendingOrder.current = true;
        onPendingChange(true);
  ...
  const handleDragStart = useCallback((event: DragEvent<HTMLButtonElement>, sectionId: string) => {
    setDraggedSectionId(sectionId);
    event.dataTransfer.effectAllowed = 'move';
    event.dataTransfer.setData('text/plain', sectionId);
  }, []);

  const handleDragOver = useCallback((event: DragEvent<HTMLDivElement>, sectionId: string) => {
    event.preventDefault();
    event.dataTransfer.dropEffect = 'move';
    setDropTargetSectionId(sectionId);
  }, []);

  const handleDragLeave = useCallback((event: DragEvent<HTMLDivElement>, sectionId: string) => {
    if (!event.currentTarget.contains(event.relatedTarget as Node | null)) {
      setDropTargetSectionId((current) => (current === sectionId ? null : current));
    }
  }, []);
  ...
  const handleDrop = useCallback(
    ...
    [draggedSectionId, reorderSection, saveOrder, autoSave, onPendingChange, hasPendingChanges]
  );
  ...
  const handleAddSection = useCallback(
    async (type: SectionType) => {
      ...
      try {
        const res = await fetch(`/api/resumes/${resumeId}/sections`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ type, content, layout: 'layout1', order_index: sections.length }),
        });
        if (!res.ok) {
    ...
    [addingSectionType, autoSave, resumeId, sections.length, addSection, hasPendingChanges, onPendingChange, setIsSaving]
  );

  return (
    <div className="resume-print-root max-w-198.5 mx-auto bg-white shadow-lg rounded-lg p-4 sm:p-8 md:p-12 min-h-0 md:min-h-280.75">
      <div className="space-y-6 sm:space-y-8">
        {visibleSections.map((section, index) => (
          <div
            key={section.id}
            className={`relative pt-10 sm:pt-8 rounded-lg transition-all ${
          ...
          >
            {section.type !== 'header' && (
              <h2 className="text-sm font-bold uppercase tracking-widest text-gray-400 mb-3 border-b border-gray-200 pb-1">
        ...
        {addingSectionType && <SectionSkeleton label={SECTION_LABELS[addingSectionType]} />}
      </div>
      ...
      <div className="no-print mt-8">
        ...
        />
        {sectionAddError && (
          <p className="mt-2 text-center text-xs text-red-500">{sectionAddError}</p>
        )}
      </div>
    </div>
  );
